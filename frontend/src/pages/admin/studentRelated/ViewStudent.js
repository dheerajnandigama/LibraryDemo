...
const ViewStudent = () => {
  ...
  useEffect(() => {
    if (userDetails) {
      ...
      setSubjectEnrolled(userDetails.subjects || []);
    }
  }, [userDetails]);
  ...
  const removeSubAttendance = (subId) => {
    dispatch(updateStudentFields(studentID, { subId }, "RemoveStudentSub")).then(() => {
      dispatch(getUserDetails(studentID, address));
    });
  };
  ...
  const subjectData = subjectEnrolled.map((entry) => {
    return {
      subject: entry.subName,
  ...
  const StudentAttendanceSection = () => {
      ...
      return (
        <>
          ...
          <Table>
            ...
            {subjectEnrolled.map((entry, index) => {
              return (
                <TableBody key={index}>
                  <StyledTableRow>
                    <StyledTableCell>{entry.subName.subName}</StyledTableCell>
                    <StyledTableCell>{entry.subName.subCode}</StyledTableCell>
                    <StyledTableCell>{entry.subName.teacher.name}</StyledTableCell>
                    <StyledTableCell>{entry.subName.sclassName.sclassName}</StyledTableCell>
                    <StyledTableCell align="center">
                      {/* <Button variant="contained" onClick={() => handleOpen(subId)}>
                        {openStates[subId] ? <KeyboardArrowUp /> : <KeyboardArrowDown />}Details
                      </Button> */}
                      <IconButton onClick={() => removeSubAttendance(entry.subName._id)}>
                      ...
                      {/* <Button
                        ...
                        onClick={() =>
                          navigate(`/Admin/subject/student/attendance/${studentID}/${subId}`)
                        }
                      >
                        Change
                      </Button> */}
    ...
    return (
      <>
        {subjectEnrolled && Array.isArray(subjectEnrolled) && subjectEnrolled.length > 0 ? (
          <>
