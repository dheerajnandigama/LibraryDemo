...
const allSubjectsByTeachers = async (req, res) => {
  try {
...
const subjectByStudent = async (req, res) => {
  try {
    const student = await Student.findById(req.params.id);

    if (student) {
      const subjectList = student.subjects.map((each) => each.subName);
      let subjects = await Subject.find({ _id: { $in: subjectList } }).populate(
        "sclassName",
        "sclassName"
      );
      if (subjects.length > 0) {
...
const setSyallabus = async (req, res) => {
  try {
